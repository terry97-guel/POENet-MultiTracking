...
class POELayer(nn.Module):
    def __init__(self, nJoint, _useAdjoint):  # output dim = 6 (log se3)
        ...
        _nominalTwist = revoluteTwist(torch.rand(nJoint, 6))        # for revolute Joint
        # _nominalTwist = prismaticTwist(torch.rand(nJoint, 6))     # for prismatic Joint

        # learnable parameter
        if _useAdjoint:
            # self.eta = nn.Parameter(torch.Tensor(nJoint, 6))        # for nonminimized joint
            self.eta = nn.Parameter(torch.Tensor(nJoint, 4))      # for revolute Joint
            # self.eta = nn.Parameter(torch.Tensor(nJoint,2))         # for prismatic Joint
        else:
        ...
        self.register_buffer('basis', self.getRevoluteBasis(_nominalTwist))     # for revolute Joint
        # self.register_buffer('basis',self.getPrismaticBasis(_nominalTwist))     # for prismatic Joint
    ...
    def getRevoluteBasis(self, twist):
        ...
        # basis = twist.new_empty(nJoint, 6, 5)  # for nonmimized joint eta is 6 dim
        perpendicular = twist.new_empty(2, 6)
        # perpendicular = twist.new_empty(1, 6)  # for nonminimized joint
        ...
        # basis = torch.broadcast_to(torch.eye(6,6),(nJoint,6,6)).to(torch.device('cuda'))
        return basis

    def getPrismaticBasis(self,twist):
        nJoint = len(twist)
        basis = twist.new_empty(nJoint,6,2)   # eta is 2 dim
        perpendicular = twist.new_empty(4,6)
        for i in range(nJoint):
            w = twist[i, :3]
            v = twist[i, 3:]
            perpendicular[:3,:3] = torch.eye(3,dtype=float)
            perpendicular[3,3:] = v
            basis[i, :, :] = getNullspace(perpendicular)
        return basis
    ...
    def updateNominalTwist(self):
        if self.useAdjoint:
            ...
            self.basis = self.getRevoluteBasis(self.nominalTwist).detach()      # for revolute Joint
            # self.basis = self.getPrismaticBasis(self.nominalTwist).detach()     # for prismatic Joint
            # self.eta.data.fill_(1e-20)
            self.eta = nn.Parameter(torch.zeros_like(self.eta))
        else:
            pass
    ...
    def getJointTwist(self):
        if self.useAdjoint:
            ...
            # expEta = expSE3(self.basis @ self.eta.view(nJoint, 6, 1))  # (nBatch,6,6)   # for nonminimized joint
            expEta = expSE3(self.basis @ self.eta.view(nJoint, 4, 1))  # (nBatch,4,4)   # for revolute Joint
            # expEta = expSE3(self.basis @ self.eta.view(nJoint, 2, 1))  # (nBatch,2,2)   # for prismatic Joint
...
class EulerSpiralModel(BaseModel):
    ...
    def getJointAngle(self, x):
        for i in range(len(self.jointLayer) - 1):
            # x = torch.nn.ReLU()(self.jointLayer[i](x))
            x = torch.tanh(self.jointLayer[i](x))
            # x = torch.nn.LeakyReLU(0.1)(self.jointLayer[i](x))
        x = self.jointLayer[-1](x)
        return x
