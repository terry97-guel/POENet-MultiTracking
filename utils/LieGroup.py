...
def getNullspace(tensor2dim):
    ...
    # not_zero = S != torch.zeros_like(S)
    # rank = int(sum(S))

    # return V[rank:, :].T.cpu().conj()
    return V[:, rank:]

def alterTwist(twist):
    nJoint, mustbe6 = twist.shape
    if mustbe6 != 6:
        print(f'[ERROR] revoluteTwist: twist.shape = {twist.shape}')
        exit(1)
    w = torch.zeros_like(twist[:, :3])
    v = twist[:, 3:]
    v_normalized = v / v.norm(dim=1).view(nJoint, 1)
    proejctedTwists = torch.empty_like(twist)
    proejctedTwists[:, :3] = w
    proejctedTwists[:, 3:] = v_normalized
    return proejctedTwists

def prismaticTwist(twist):
    nJoint, mustbe6 = twist.shape
    if mustbe6 != 6:
        print(f'[ERROR] revoluteTwist: twist.shape = {twist.shape}')
        exit(1)
